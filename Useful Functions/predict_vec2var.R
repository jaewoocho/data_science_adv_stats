#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
#### Predict New Data | `vec2var ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#

# I have adapted a function to predict new
# data with a `vec2var` model. This function
# was obtained from the {vars} package and
# adapted to handle new data.
#
# The source code that this function was adapted
# from is available here:
#
# https://github.com/cran/vars/blob/master/R/predict.vec2var.R
#
# Any errors are mine and not the author(s) of {vars}

predict_vec2var <- function(object, ..., newdata = NULL, n.ahead = 10, ci = 0.95, dumvar = NULL){
  ...
  # Internal function brought into this modified function
  ".fecovvec2var" <-
    function(x, n.ahead) {
  ...
  if(!is.null(newdata)){
    ## forecast newdata
    forecast <- matrix(NA, ncol = K, nrow = nrow(newdata))
    
    ## make newdata a matrix
    new_data <- as.matrix(newdata)
    
    ## add lags to new data
    new_data <- new_data[,!is.na(match(colnames(new_data), colnames(Zy)))]
    ...
    ## add lags
    for(i in 1:p){
      lagged <- rbind(NA, new_data)
      colnames(lagged) <- paste0(
        colnames(new_data), ".l", i
      )
      if(i == 1){
        new_lagged_data <- lagged[-nrow(lagged),]
      }else{
        new_lagged_data <- cbind(new_lagged_data, lagged[-nrow(lagged),])
      }
    }
    
    ## add to lagged data to new data
    new_data <- cbind(new_data, new_lagged_data)
